...
async function Get (people, questions, dates) {
    return new Promise(async resolve => {
        ...
        // TODO: Seperate the following lines to calls from the service to the correct model 
        //******** 5: Get the Questions ********
        query = `SELECT q.id, q.text, q.created FROM q `;
        if(!questions.includes(-202)) {
            query += "WHERE "
            questions.forEach(q => query += `q.text LIKE "%${q}%" OR `);
            query = query.substring(0, query.length - 3);
        }
        console.log(query)
        ...
        if(!people.includes(-202)) {
            query += "WHERE "
            people.forEach(p => query += `u.name LIKE "%${p}%" OR u.email LIKE "%${p}%" OR `);
            query = query.substring(0, query.length - 3);
        }
        console.log(query)
        ...
        resolve({ 
            ...
            responses: responseData.resources, 
            questions: questionData.resources, 
